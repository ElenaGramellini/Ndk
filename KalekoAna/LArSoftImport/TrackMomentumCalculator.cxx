...
// Look for leonidas technical notes!!!!!
// In reco algo package (larreco) note 4140
...
namespace larlite {
  ...
  Double_t TrackMomentumCalculator::GetMuMultiScatterLLHD2( const larlite::mctrack &trk )
  {
    ...
    //Loop on the track points to store them in X,Y,Z vectors
    ...
    //////Here a series of checks starts
    // Is the track long enough?
    Int_t my_steps = recoX.size();
    //std::cout<<"kaleko: my_steps = "<<my_steps<<" "<<trk.size()<<std::endl;
    if ( my_steps<2 ) return -1.0;
 
    // Does recoX, recoY, recoZ have the same size? 
    // If they do, fill gr_reco_xyz, gr_reco_xy, gr_reco_yz, gr_reco_xz
    ...
    // Maybe this means:
    // momentum range = reconstructed length * mip energy deposition?
  ...
  Int_t TrackMomentumCalculator::GetRecoTracks( const std::vector<Float_t> &xxx, const std::vector<Float_t> &yyy, const std::vector<Float_t> &zzz )
  {
    ...
    gr_reco_yz = new TGraph( n_reco, z_reco, y_reco ); 
    gr_reco_xz = new TGraph( n_reco, z_reco, x_reco ); 
    gr_reco_xy = new TGraph( n_reco, x_reco, y_reco );
