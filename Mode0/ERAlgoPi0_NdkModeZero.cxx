#ifndef ERTOOL_ERALGOPI0_NDKMODEZERO_CXX
#define ERTOOL_ERALGOPI0_NDKMODEZERO_CXX
...
namespace ertool {
  ...
  ERAlgoPi0_NdkModeZero::ERAlgoPi0_NdkModeZero(const std::string& name) 
    : AlgoBase(name)
    , _radLenVar(nullptr)
    , _vtxVar(nullptr)
    , _vtxSigl(nullptr)
  ...
  void ERAlgoPi0_NdkModeZero::SetDefaultParams()
  {
  ...
  void ERAlgoPi0_NdkModeZero::AcceptPSet(const ::fcllite::PSet& cfg) {
  ...
  void ERAlgoPi0_NdkModeZero::ProcessBegin()
  {
  ...
  void ERAlgoPi0_NdkModeZero::LL(const Shower& shower_a,
...
//      return;
  ...
  bool ERAlgoPi0_NdkModeZero::Reconstruct(const EventData &data, ParticleGraph& graph)
  { 
...
//    std::cout<<"\n\n\nNew Event! "<<std::endl ;
  ...
  std::cout<<"Shower 2: "<<abs(graph.GetParticle(shrID2).PdgCode())<<std::endl;
	...
	if ( (likelihood != -1.e-10) && (likelihood > -10)&& (mass > 0)&& shr1._energy > 20 && shr2._energy > 20 ){
	  ...
	  // Add relationships
    //std::cout<<"Shower PDGs: "<<graph.GetParticle(shrID1).PdgCode()<<" "<<graph.GetParticle(shrID2).PdgCode()<<std::endl;
...
//	  _mgr.Add (s, ::ertool::RecoInputType_t(0,data.Shower().name()), true i);
  ...
  void ERAlgoPi0_NdkModeZero::ProcessEnd(TFile* fout)
  {
